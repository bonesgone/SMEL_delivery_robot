...
class RobotController(Node):
    ...
    def robot_controller_callback(self):
        ...
        if self.get_clock().now() - self.start_time > Duration(seconds=DELAY) and self.scan_available:
            left_scan_min = min(self.laserscan[27:55]) # Minimum of laserscan from left sector
            right_scan_min = min(self.laserscan[135:162]) # Minimum of laserscan from right sector
